...
std::vector<Photon> response_transport(Source &source, const Mesh &mesh,
  ...
  while (n_local_sourced < n_local) {
    ...
    event = resp_transport_photon(phtn, mesh, rng, next_dt, dt, exit_E,
                             census_E, abs_E, rank_abs_E, rank_track_E, tally, resp);
  ...
  double conservation = (mesh.get_total_photon_E()-exit_E-census_E-abs_E)/mesh.get_total_photon_E();
  std::cout<<"CONSERVATION "<<conservation<<std::endl;
